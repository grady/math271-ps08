...
1. Use the code `vole_trapping <- read_csv("http://bit.ly/2IgDF0E")` to import a new data set. Make an initial scatterplot of the `vole_trapping` data, using `veg` as the x-axis, `voles` as the y-axis, and `soil` as the color.

2. Fit a main-effects only model to the vole data. Generate predictions and add the prediction lines to the scatterplot. The lines should be parallel.

3. Fit a model with interaction between the predictors. Generate predictions and add the prediction lines to a new scatterplot.
